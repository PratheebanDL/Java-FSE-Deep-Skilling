-- Country reference data. Original values converted from double-quoted
-- string literals (invalid in standard SQL/H2) to properly single-quoted,
-- escaped literals so this script executes correctly against H2.
-- Existing rows are cleared first, per the task's population instructions.
DELETE FROM country;
