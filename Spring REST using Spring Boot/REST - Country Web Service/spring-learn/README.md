...
A small Spring MVC (classic XML configuration) web application that exposes
one REST endpoint, `/country`, which returns India's details as JSON,
loaded from a bean declared in Spring XML config.
